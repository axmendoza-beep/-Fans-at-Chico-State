...
function ProfileSetup() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setError('Unable to load profile information: ' + err.message);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage('Profile saved! Redirecting you to the home page...');
      navigate('/');
    } catch (err) {
      setError('Error saving profile: ' + (err.response?.data?.error || err.message));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div style={{ maxWidth: '500px', margin: '2rem auto' }}>
      ...
      <p>Please confirm your email and choose a username. You can optionally add more details now.</p>
